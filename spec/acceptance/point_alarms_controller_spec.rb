...
resource "告警相关" do
  ...
  get "/rooms/:room_id/point_alarms" do
    before(:each) do
      create(:user)
      @room = create(:room)
      ...
      (0..3).each do |i|
        ...
        (0..i).each do |index|
          ...
          point_alarm = create(:point_alarm, point: point, room: @room, device: point.device)
        end
    ...
    response_field :pid, "父节点id"
    response_field :meaning, "告警状态"
    response_field :room_id, "房间id"
    response_field :meaning, "告警状态"
    response_field :reported_at, "创建时间"
    response_field :cleared_at, "解除时间"
    response_field :checked_at, "确认时间"
    response_field :is_checked, "是否确认"
    response_field :is_cleared, "是否解除"
  ...
  get "/rooms/:room_id/sub_systems/:sub_system_id/point_alarms" do
    before(:each) do
      create(:user)
      @room = create(:room)
      @sub_system = create(:sub_system)
  ...
  get "/rooms/:room_id/point_alarms/count" do 
    before(:each) do
      create(:user)
      @room = create(:room)
      @points = []
