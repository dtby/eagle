...
class PointAlarmsController < BaseController
  ...
  def index
    ...
    if params[:checked].present? && params[:checked] == "0"
      @point_alarms = point_alarms.paginate(page: page, per_page: (params[:per_page] || 10)).sort_by {|p| p.point.name[/\d+/].to_i }
    elsif params[:checked].present? && params[:checked] == "1"
      @point_alarms = point_alarms.select{ |pa| pa.is_checked }.paginate(page: page, per_page: (params[:per_page] || 10)).sort_by {|p| p.point.name[/\d+/].to_i }
    else
      @point_alarms = point_alarms.select{ |pa| (!pa.is_checked) }.paginate(page: page, per_page: (params[:per_page] || 10)).sort_by {|p| p.point.name[/\d+/].to_i }
    end
  end
